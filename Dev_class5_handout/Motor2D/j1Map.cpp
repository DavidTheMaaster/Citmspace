...
bool j1Map::Load(const char* file_name)
{
	...
	// Load layer info ----------------------------------------------
	pugi::xml_node layerinfo;
	for (layerinfo = map_file.child("map").child("tileset"); layerinfo && ret; layerinfo = layerinfo.next_sibling("tileset"))
	{
		MapLayer* set = new MapLayer();

		if (ret == true)
		{
			ret = LoadLayer(layerinfo, set);
		}
